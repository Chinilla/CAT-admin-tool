...
from chinilla.types.spend_bundle import CoinSpend, SpendBundle
...
async def app(chinilla_config, chinilla_root, secure_the_bag_targets_path: str, leaf_width: int, tail_hash_bytes: bytes32, unwind_target_puzzle_hash_bytes: bytes32, genesis_coin_id: bytes32, fingerprint:int, wallet_id:int, unwind_fee: int):
    full_node_client = await FullNodeRpcClient.create(chinilla_config["self_hostname"], chinilla_config["full_node"]["rpc_port"], chinilla_root, load_config(chinilla_root, "config.yaml"))
    wallet_client = await WalletRpcClient.create(chinilla_config["self_hostname"], chinilla_config["wallet"]["rpc_port"], chinilla_root, load_config(chinilla_root, "config.yaml"))
...
@click.option(
    ...
    default=500000,
    show_default=True,
    help="Fee paid for each unwind spend. Enough vojos must be available to cover all spends.",
)
@click.option(
    "-lw",
    "--leaf-width",
...
def cli(
    ...
    chinilla_root: Path = Path(os.path.expanduser(os.getenv("CHINILLA_ROOT", "~/.chinilla/vanillanet"))).resolve()
    chinilla_config = load_config(chinilla_root, "config.yaml")

    asyncio.get_event_loop().run_until_complete(
        app(chinilla_config, chinilla_root, secure_the_bag_targets_path, leaf_width, tail_hash_bytes, unwind_target_puzzle_hash_bytes, eve_coin_id, fingerprint, wallet_id, unwind_fee)
    )
